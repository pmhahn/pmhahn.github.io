...
All variants below will be based on the following program frame, where `strXcpy` is replaced by the corresponding variant
...
More variants exists:

- specify length by _number of characters_ or _end of buffer_
- return _number of characters copied_ vs. _return buffer head_ vs. _next position after last character written_ AKA _next character_.
- walk source buffer _once_ for copying or _twice_ for `strlen()` first and then again for copying.

<man:string_copying(7)> lists many of them and has a lot more to say about their pros and cons.
