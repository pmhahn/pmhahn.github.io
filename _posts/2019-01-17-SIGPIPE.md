...
On a system using *LDAP* as back-end for <man:getent> *NSS* used *libldap* to open a connection to the LDAP server.
Internally it uses *liblber* for serializing the data, which uses <man:write(2)> to send its data.
If something goes wrong their, your process will get the `SIGPIPE` and terminate.
